      ...
      <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm">تبدأ الكلمة بآخر حرف من الكلمة السابقة</p>
          </div>
        </div>
          ...
          {status === 'setup' && (
            ...
            >
              ...
              <div className="relative z-10">
                <UserPlus className="w-32 h-32 text-blue-500 mx-auto mb-8 drop-shadow-[0_0_20px_rgba(59,130,246,0.3)]" />
                <h3 className="text-3xl font-bold text-white mb-4">إعداد اللعبة</h3>
                <p className="text-zinc-400 mb-12 text-lg">اطلب من المتابعين كتابة <span className="text-blue-400 font-bold">!join</span> للمشاركة. نحتاج لاعبين على الأقل.</p>
                
                <button
                  onClick={startGame}
                  disabled={joinedPlayers.length < 2}
                  className="w-full flex items-center justify-center gap-3 py-6 bg-blue-600 hover:bg-blue-500 disabled:bg-zinc-800 disabled:text-zinc-500 text-white rounded-2xl font-bold text-2xl transition-all shadow-[0_0_30px_rgba(37,99,235,0.4)] hover:shadow-[0_0_40px_rgba(37,99,235,0.6)] active:scale-95"
                >
            ...
            <div className="flex flex-col items-center w-full">
              ...
              >
                {currentWord.split('').map((char, i) => (
                  <span 
                    key={i} 
                    className={i === currentWord.length - 1 ? "text-blue-400 glow-blue-text" : ""}
                  >
                    {char}
                  </span>
                ))}
              ...
              <div className="text-2xl text-zinc-300 mb-12">
                الحرف المطلوب: <span className="text-blue-400 font-bold text-4xl mx-2">{currentWord.slice(-1)}</span>
              </div>

              <div className="bg-white/5 border border-white/10 rounded-3xl p-8 w-full max-w-md text-center">
                <div className="text-zinc-400 text-sm font-bold uppercase tracking-wider mb-2">دور اللاعب</div>
          ...
          {status === 'finished' && (
            ...
            >
              ...
              {activePlayers.length === 1 && (
                <p className="text-2xl text-yellow-500 font-bold mb-4">الفائز: {activePlayers[0]}</p>
              )}
                ...
                activePlayers.map((username, index) => (
                  <motion.div
                    ...
                    className={`border rounded-xl p-4 flex items-center justify-between transition-colors ${
                      index === currentPlayerIndex ? 'bg-blue-500/20 border-blue-500/50' : 'bg-white/5 border-white/10 opacity-60'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-white truncate max-w-[120px]">{username}</span>
                      {index === currentPlayerIndex && <span className="text-xs text-blue-400 font-bold">دوره</span>}
                    </div>
                    <span className="text-blue-400 font-bold">{scores[username] || 0}</span>
                  </motion.div>
                ))
              )}
