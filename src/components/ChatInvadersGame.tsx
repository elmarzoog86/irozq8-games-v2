...
export const ChatInvadersGame: React.FC<Props> = ({ messages, onLeave }) => {
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto bg-black/40 backdrop-blur-md rounded-3xl border border-white/10 overflow-hidden shadow-2xl">
      {/* Header */}
      <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm">دافع عن سفينتك من رسائل المتابعين!</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative">
        {status === 'setup' && (
          <motion.div 
            ...
            className="w-full max-w-md bg-white/5 p-8 rounded-3xl border border-white/10 text-center"
          >
            <Skull className="w-24 h-24 text-blue-500 mx-auto mb-6" />
            ...
            <button
              onClick={startGame}
              className="w-full flex items-center justify-center gap-2 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-lg transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] hover:shadow-[0_0_30px_rgba(37,99,235,0.5)]"
            >
        ...
        {status === 'playing' && (
          <div className="relative w-full max-w-[800px] aspect-[4/3] bg-black/80 rounded-2xl overflow-hidden border border-white/10 shadow-inner">
            <canvas 
              ref={canvasRef} 
              width={800} 
              height={600} 
        ...
        {status === 'gameover' && (
          ...
          >
            ...
            <p className="text-2xl text-blue-400 font-bold mb-8">النقاط: {score}</p>
