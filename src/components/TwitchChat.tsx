  ...
  return (
    <div className="flex flex-col h-full bg-black/40 backdrop-blur-xl font-arabic" dir="rtl">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className="group text-sm break-words animate-in fade-in slide-in-from-bottom-2 bg-white/5 p-3 rounded-2xl border border-white/5 hover:border-brand-gold/20 transition-all">
            <span className="font-black text-brand-gold ml-2 tracking-tight">{msg.username}</span>
            <span className="text-brand-gold/70 font-medium">{msg.message}</span>
          </div>
        ))}
